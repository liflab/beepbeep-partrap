...
/**
 * This class allows to verify the fifteenth property of the
 * project (refer to the pdf included in the project)
...
public class CheckTrackersConnection extends GroupProcessor
{
  ...
  /**
   * The ApplyFunction allow to define the constant state that allow to compare
   * it with the JSON filed with key "state"
   */
  ...
  public CheckTrackersConnection()
  {
    ...
    this.addProcessors(m_fork, m_getState, m_constState, m_containStateValue, m_filter, m_getTime,
        m_checkTimeBetweenLastEvent);
  ...
  /**
   * This inner class allow to verify if time between the last event with s_stateValue state is
   * less that constant: s_timeLimitBetweenTwoEvent
   * @author Helloïs BARBOSA
   *
   */
