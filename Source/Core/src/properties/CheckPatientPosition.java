...
/**
 * This class allows to verify the fourth property of the project (refer to the pdf included in the project): absence_of set ( HipCenter hc , KneeCenter kc ) where dist ( hc . point , kc . point ) <= d
...
public class CheckPatientPosition extends GroupProcessor {
	...
	/**
	 * The Fork to split stream in two parts: one to filter HipCenter and the other one to filter KneeCenter
	 */
	...
	/**
	 * The ApplyFunction allow to define the constant id that allow to compare it with the JSON filed with key "id"
	 */
	...
	/**
	 * The ApplyFunction allow to get the field with the key: "point" that correspond to a JsonList that contain 3D points of a HipCenter JsonObject
	 */
	...
	/**
	 * The ApplyFunction allow to get the field with the key: "point" that correspond to a JsonList that contain 3D points of a KneeCenter JsonObject
	 */
	private ApplyFunction m_getKneeCenterPoint;
	
	
	/**
	 * This ApplyFunction allow to check if a set of Hip and Knee points are at the right distance from each other (meaning that is, by calling the inner BinaryFunction: CheckHipAndKneePoints)
	 */
	...
	public CheckPatientPosition(Float minHipKneeDist) {
		...
		Connector.connect(m_firstFork, 0, m_hipCenterFork, 0);
			...
			//First branch of the m_hipCenterFork (output: 0): here we just pass JsonElement to m_filterHipCenter
		...
		Connector.connect(m_firstFork, 1, m_kneeCenterFork, 0);
			...
			//First branch of the m_kneeCenterFork (output: 0): here we just pass JsonElement to m_filterKneeCenter
			m_filterKneeCenter = new Filter();
			Connector.connect(m_kneeCenterFork, 0, m_filterKneeCenter, 0);
			
			
			//Second branch of the m_hipCenterFork (output: 1): here we check that the event id is KneeCenter
		...
		m_checkHipAndKneePoints = new ApplyFunction(new CheckHipAndKneePoints(minHipKneeDist));
		...
		this.addProcessors(m_hipCenterFork, m_filterHipCenter, m_getIdHipCenter, m_constIdHipCenter, m_equalIdHipCenter, m_getHipCenterPoint);
		this.addProcessors(m_kneeCenterFork, m_filterKneeCenter, m_getIdKneeCenter, m_constIdKneeCenter, m_equalIdKneeCenter, m_getKneeCenterPoint);
		this.associateInput(0,  m_firstFork, 0);
		this.associateOutput(0, m_checkHipAndKneePoints, 0);
	}
	
	/**
	 * This inner class is a BinaryFunction that it take a two JsonElement (meaning two JsonList of 3D points) and give Boolean (with d the distance between the two 3D points: d > m_minHipKneeDist)
	 * @author Helloïs BARBOSA
	 *
	 */
	private class CheckHipAndKneePoints extends BinaryFunction<JsonElement, JsonElement, Boolean>{
		 ...
		 * 							The minimum distance that there must be between an HipCenter point and a KneeCenter point
		 */
		public CheckHipAndKneePoints(Float minHipKneeDist) {
			super(JsonElement.class, JsonElement.class, Boolean.class);
			m_minHipKneeDist = minHipKneeDist;
		}
		...
		public Boolean getValue(JsonElement arg0, JsonElement arg1) {
			if(!(arg0 instanceof JsonNull) && !(arg1 instanceof JsonNull)) {
				...
				Float[] coordHipCenter = new Float[] {((JsonNumber)hipCenter.get(0)).numberValue().floatValue(), ((JsonNumber)hipCenter.get(1)).numberValue().floatValue(), ((JsonNumber)hipCenter.get(2)).numberValue().floatValue()};
				Float[] coordKneeCenter = new Float[] {((JsonNumber)kneeCenter.get(0)).numberValue().floatValue(), ((JsonNumber)kneeCenter.get(1)).numberValue().floatValue(), ((JsonNumber)kneeCenter.get(2)).numberValue().floatValue()};
				
				System.out.println("Distance: " + UtilityMethods.distanceProcessing(coordHipCenter, coordKneeCenter));
