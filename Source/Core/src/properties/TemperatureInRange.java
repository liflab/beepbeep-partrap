...
public class TemperatureInRange extends GroupProcessor {
	...
	/**
	 * The ApplyFunction allow to get the field with the key: "v1" that correspond to the value of the temperature
	 */
	...
	/**
	 * The ApplyFunction allow to check if the temperature is between a range (meaning that is, by calling the inner BinaryFunction: IsBetween)
	 */
	private ApplyFunction m_checkIsBetween;
	
	/**
	 * The ApplyFunction allow to check if the both condition of this property are verified and give the final result
	 */
	 ...
	 */
	public TemperatureInRange(float lowestValue, float highestValue) {
		super(1, 1);
		...
		//Second branch of the fork (output: 1): here we check that the temperature field is the right one
		m_getTemperature = new ApplyFunction(new GetJsonFields(GetJsonFields.JK_TEMP));
		Connector.connect(m_fork, 1, m_getTemperature, 0);
		
		m_constRange = new ApplyFunction(new Constant(new Float[] {lowestValue, highestValue}));
		...
		//We apply an AND logic between the two results because we need two answer to be true to verify this property
		...
		this.addProcessors(m_fork, m_getId, m_constId, m_equalId, m_getTemperature, m_constRange, m_checkIsBetween, m_logicAnd);
